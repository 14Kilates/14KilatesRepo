...
addon = xbmcaddon.Addon('plugin.video.14K')
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.14K-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification([COLOR red]*****14 KILATES*****[/COLOR],We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification([COLOR red]*****14 KILATES*****[/COLOR],We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def getSources():
        try:
            ...
            if addon.getSetting("browse_community") == "true":
                addDir('                     [B][COLOR gold]********BIENVENIDOS A 14 KILATES********[/COLOR][/B]','community_files',16,icon,FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(14 KILATES,New source added.,5000,"+icon+")")
        if not url is None:
...
def getCommunitySources(browse=True):
        if logged_in == False:
            url = 'https://svn.code.sf.net/p/userincorrecto14/svn/ACCESO_NO_AUTORIZADO'
        else:
            url = logged_in
        soup = BeautifulSoup(makeRequest(url), convertEntities=BeautifulSoup.HTML_ENTITIES)
                ...
                if len(url) > 1:
                    ...
                    if isXMLSource:
                            if not regexs == None: #<externallink> and <regex>
                                ...
                                #addLink(url[0],name.encode('utf-8', 'ignore')+  '[COLOR gold]build XML[/COLOR]',thumbnail,fanArt,desc,genre,date,True,None,regexs,total)
                            else:
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(14 KILATES,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('14 KILATES','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('14 KILATES', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from 14 KILATES Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if showcontext == '!!update':
                ...
                contextMenu.append(('[COLOR gold]!!update[/COLOR]','XBMC.RunPlugin(%s)' %fav_params2))
            if not name in FAV:
                contextMenu.append(('','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def ytdl_download(url,title,media_type='video'):
    ...
    if not url == '':
        ...
        xbmc.executebuiltin("XBMC.Notification(DOWNLOAD,First Play [COLOR gold]WHILE playing download[/COLOR] ,10000)")
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from 14 KILATES Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to 14 KILATES Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(14 KILATES,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(14 KILATES,Please [COLOR gold]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(14 KILATES,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(14 KILATES,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(14 KILATES,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
